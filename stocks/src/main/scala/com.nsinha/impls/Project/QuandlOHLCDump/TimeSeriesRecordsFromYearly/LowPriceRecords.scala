...
import com.nsinha.impls.Project.TimeSeries.{ClosingPriceTimeSeries, HighPriceTimeSeries, LowPriceTimeSeries, OpenPriceTimeSeries}
import com.nsinha.utils.FileUtils

/** Created by nishchaysinha on 10/21/16.
  */
object LowPriceRecords extends YearlyRecordsTrait {
  def clazz(x : String, admitTickers : List[String]) = new LowPriceTimeSeries(x, admitTickers = admitTickers)
  override def processYears(years : List[Int], jsonFileNameRootDir : String = "/Users/nishchaysinha/stocksdatadir/ohlc/yearlies",
                            inputFileRelative : String = "output/combinedData.json", outputFileRelative : String = "output/lowprice.json", admitTickers : List[String]) = {
    years map { year ⇒
      (new YearlyRecords) (
  ...
  override def processDirectory(dir : String, outFileDirRelative : String = "output/lowprice", admitTickers : List[String]) = {
    ...
    val jsonFileName = outDir+"/"+"lowprice.json"

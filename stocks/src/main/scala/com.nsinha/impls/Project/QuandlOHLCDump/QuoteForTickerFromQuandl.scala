...
object QuoteForTickerFromQuandl extends QuoteForTickerFromQuandlTrait with Loggable {
  ...
  private def convertToQuotesArray(ticker : String, dataArray : JsonNode, mapper : Map[String, String] = Map(DATE → "index 0", "datetimeend" → "index 0", HIGH → "index 2", LOW → "index 3", OPEN → "index 1", CLOSE → "index 4", "volume" → "index 5", PREV → "index 1"), fn : (String, JsonNode, Map[String, String]) ⇒ GenCsvQuoteRowScottrade) : List[GenCsvQuoteRowScottrade] = {
